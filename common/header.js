...
export function renderHeader({ title="Arcade", backTo="./", hideBack=false }){
  ...
  if(!hideBack){const b=document.createElement("button");b.className="back";b.textContent="← Arcade";b.onclick=()=>location.href=backTo;bar.appendChild(b);}
  ...
  const pts=document.createElement("div");pts.className="points";const label=document.createElement("span");label.textContent="Points";
  const val=document.createElement("strong");val.textContent=formatPoints(points.ensureInit(1000));
  pts.appendChild(label);pts.appendChild(val);bar.appendChild(pts);document.body.prepend(bar);
