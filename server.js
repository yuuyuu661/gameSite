...
function awardPoints(db, player, game, score) {
  const multipliers = { aim:2, memory:1, numbers:1, fusion:2, block:1 };
...
app.get("*", (req,res)=>{ const fp=path.join(__dirname,"public",req.path); if(fs.existsSync(fp)){res.sendFile(fp);} else {res.sendFile(path.join(__dirname,"public","index.html"));} });
