class ArcadeWallet {
  ...
  get(){ const v = this.getRaw(); return v ? parseInt(v,10) : 0; }
  set(n){ localStorage.setItem(this.key, String(Math.max(0, Math.floor(n))));
    window.dispatchEvent(new CustomEvent("points:change", { detail:{ value:this.get() } })); }
  add(d){ this.set(this.get()+d); }
  ensureInit(defaultValue=1000){ if(this.getRaw()===null){ this.set(defaultValue); } return this.get(); }
}
